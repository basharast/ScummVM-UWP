...
bool showTip = false;
bool loaded = false;
extern "C" bool getLoadState() {
	return loaded;
}
extern "C" void setTipState(bool tipState) {
	showTip = tipState;
}
...
#endif
	try {
		//auto deviceID = ConfMan.get("deviceType");
		if (showTip) {
			GUI::MessageDialog dialog(("To show main menu inside the game\nUse two fingers like 'Zoom' move"), ("OK"));
			dialog.runModal();
		}
	}
	catch (...) {
